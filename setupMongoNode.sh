...
# To try again, delete the /etc/mongod.conf file and stop the 
# mongod service if it has run before + blow away the db data.
#

echo Setting up MongoDB on Windows Azure
echo
...
chmod +x nave.sh
sudo ./nave.sh usemain 0.10.28  > /tmp/naveNodeUseMain.log 2>&1
# ./nave.sh install 0.10.28
# ./nave.sh use 0.10.28

nodeInstalled=$(node -v)
if [ "$nodeInstalled" != "v0.10.28" ]; then
        echo Node.js could not be installed.
        exit 1
fi

echo Installing Windows Azure Node.js module...
npm install azure@0.9.7  > /tmp/nodeInstall.log 2>&1
...
if $isPrimary; then
	...
	read -p "Please enter a new password for the 'clusteradmin' MongoDB user: " primaryPasscode
	echo
	read -p "Please confirm that awesome new password: " primaryPasscodeConfirmation
	echo
